<template>
  <footer class="border-t border-muted mt-16 py-8">
    <div class="max-w-5xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
      <p class="font-mono text-xs text-dim">© {{ year }} {{ name }}. All rights reserved.</p>
      <div class="flex items-center gap-4">
        <a
          ...
          target="_blank"
          :title="link.label"
          class="text-dim hover:text-ink transition-colors"
        >
...
// Simple SVG icon components inline
...
const socialLinks = [
  { label: 'GitHub', url: profile.social.github, icon: GithubIcon },
  { label: 'LinkedIn', url: profile.social.linkedin, icon: LinkedinIcon },
  { label: 'Instagram', url: profile.social.instagram, icon: InstagramIcon },
]
</script>
